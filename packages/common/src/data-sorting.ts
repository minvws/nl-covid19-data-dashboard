...
import { GmSewerPerInstallationValue, NlVariantsVariantValue, VrSewerPerInstallationValue } from './types';
...
export function sortTimeSeriesInDataInPlace<T>(data: T, { setDatesToMiddleOfDay = false } = {}) {
  ...
  if (isDefined((data as UnknownObject).sewer_per_installation)) {
    ...
    nestedSeries.values = nestedSeries.values.map((x, index) => {
      ...
      x.values = sortTimeSeriesValues(x.values) as VrSewerPerInstallationValue[] | GmSewerPerInstallationValue[];
...
export interface SewerPerInstallationData {
  values: (TimeSeriesMetric<VrSewerPerInstallationValue | GmSewerPerInstallationValue> & {
