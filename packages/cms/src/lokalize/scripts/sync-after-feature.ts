...
`,
  {
    flags: {
      token: {
        type: 'string',
        default: process.env.NEXT_PUBLIC_SANITY_TOKEN?.toString(),
      },
    },
  }
);
 ...
 */
const applyDeletionsToDevelopment = async (deletions: DeleteMutation[]) => {
  ...
  const developmentTransaction = developmentClient.transaction();

  documentIdsToDelete.forEach((documentId) => developmentTransaction.delete(documentId));

  await developmentTransaction.commit();

  console.log(`Deleted ${documentIdsToDelete.length} text documents (including draft versions)`);
};

const syncAdditionsToProduction = async (mutations: AddMutation[]) => {
  ...
  for (const mutation of mutations) {
    // Because not all mutation have written a document_id in the mutations file yet, we need to workaround this with a find.
    const document = mutation.document_id
      ? ((await developmentClient.getDocument(mutation.document_id)) as LokalizeText | undefined)
      : allPublishedTexts.find((publishedText) => publishedText.key === mutation.key);
    ...
    if (document) {
      ...
      if (count === 0) {
         ...
         * set.
         */
        productionTransation.createIfNotExists(documentToInject);
        successCount++;
      } else {
  ...
  await productionTransation.commit();
