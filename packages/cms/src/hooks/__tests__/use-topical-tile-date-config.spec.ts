...
import { useTopicalTileDateConfig, TopicalDateConfig } from '../use-topical-tile-date-config';

const UseTopicalTileDateConfig = suite('useTopicalTileDateConfig');
...
UseTopicalTileDateConfig('Should retrieve the formated date for the 1st config', () => {
  ...
  const result = useTopicalTileDateConfig(dateConfigArray[0]);
  assert.is(result, expectedResult);
});

UseTopicalTileDateConfig('Should retrieve the formated date for the 2nd config', () => {
  ...
  const result = useTopicalTileDateConfig(dateConfigArray[1]);
  assert.is(result, expectedResult);
});

UseTopicalTileDateConfig('Should retrieve the formated date for the 3rd config', () => {
  ...
  const result = useTopicalTileDateConfig(dateConfigArray[2]);
  assert.is(result, expectedResult);
});

UseTopicalTileDateConfig.run();
