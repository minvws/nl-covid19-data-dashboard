 ...
 */
(function run() {
  const topology = JSON.parse(fs.readFileSync(path.join(__dirname, '../../../app/src/components/choropleth/geography-simplified.topo.json'), { encoding: 'utf-8' }));

  const gmGeo = topojson.feature(topology, topology.objects.gm_collection) as unknown as GmGeoJSON;

  const vrGeo = topojson.feature(topology, topology.objects.vr_collection) as unknown as VrGeoJSON;
  ...
  const tsFilePath = path.join(__dirname, '../../../app/src/components/choropleth/vr-bounding-box-gm-codes.ts');
