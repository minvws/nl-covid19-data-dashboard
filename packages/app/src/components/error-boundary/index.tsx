...
const PropsReportContext = createContext<() => Record<string, unknown> | undefined>(() => undefined);

type ErrorBoundaryProps = {
  children?: ReactNode;
  extraComponentInfoReport?: Record<string, unknown> | (() => Record<string, unknown> | undefined);
};

export function ErrorBoundary({ children = null, extraComponentInfoReport }: ErrorBoundaryProps) {
  const additionalProps = isFunction(extraComponentInfoReport) ? extraComponentInfoReport() : extraComponentInfoReport;

  const [extractPropsFromChildren, propsReportCallback] = useComponentPropsReport(additionalProps);
...
function ErrorFallback({ error }: { error: Error }) {
  ...
  const [clipboardState, setClipboardState] = useState<'init' | 'copied' | 'error'>('init');
  ...
  return (
    <ErrorBox>
      <Markdown
        content={replaceVariablesInText(commonTexts.common.algemene_foutmelding, {
          email_address: markdownEmail,
        })}
      />
      <Box display="flex" alignItems="center">
        <Button onClick={() => copyErrorReport()}>{commonTexts.common.kopieer_foutmelding}</Button>

        {clipboardState === 'copied' && <InlineText>{commonTexts.common.foutmelding_is_gekopieerd}</InlineText>}
        {clipboardState === 'error' && <InlineText color="red2">{commonTexts.common.foutmelding_kon_niet_gekopieerd_worden}</InlineText>}
      </Box>
...
function formatErrorReport(error: Error, componentProps?: Record<string, unknown>) {
  ...
  if (isDefined(componentProps)) {
    ...
    for (const prop in componentProps) {
      report.push(`${prop}: ${JSON.stringify(componentProps[prop], null, '\t')}`);
    }
  }
