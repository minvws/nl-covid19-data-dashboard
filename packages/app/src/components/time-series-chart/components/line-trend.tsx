...
export function LineTrend({ series, style = DEFAULT_STYLE, strokeWidth = DEFAULT_STROKE_WIDTH, color, getX, getY, curve = 'linear', id }: LineTrendProps) {
  ...
  return <LinePath data={nonNullSeries} x={getX} y={getY} stroke={color} strokeWidth={strokeWidth} curve={curves[curve]} strokeDasharray={style === 'dashed' ? 4 : undefined} strokeLinecap="round" strokeLinejoin="round" id={id} />;
}
...
export function LineTrendIcon({ color, strokeWidth = DEFAULT_STROKE_WIDTH, style = DEFAULT_STYLE, width = 15, height = 15 }: LineTrendIconProps) {
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      <line stroke={color} strokeWidth={strokeWidth} strokeDasharray={style === 'dashed' ? 4 : undefined} strokeLinecap="round" strokeLinejoin="round" x1={2} y1={height / 2} x2={width - 2} y2={height / 2} />
    </svg>
  );
}
