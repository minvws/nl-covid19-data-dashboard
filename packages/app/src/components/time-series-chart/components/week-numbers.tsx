...
export function WeekNumbers({ startUnix, endUnix, xScale, bounds }: WeekNumberProps) {
  /**
  ...
  const { weekGridLines, weekNumberLabels } = useMemo(() => calculateWeekNumberAxis(startUnix, endUnix), [startUnix, endUnix]);
   ...
   */
  const weekRenderWidth = getWeekRenderWidth(xScale(weekGridLines[2]), xScale(weekGridLines[1]));
  ...
  return (
    <>
      <RectClipPath id={id} width={bounds.width} height={bounds.height + DRAWING_BUFFER * 2} x={0} y={-DRAWING_BUFFER} />
      <g
        css={css({
          clipPath: `url(#${id})`,
        })}
...
function calculateWeekNumberAxis(startUnix: number, endUnix: number) {
   ...
   */
  const alternateBy = weekCount > MAXIMUM_WEEK_COUNT ? Math.ceil(weekCount / MAXIMUM_WEEK_COUNT) : 1;
  ...
  const weekNumberLabels = filterWeeks(weekGridLines, alternateBy, startUnix + 3 * DAY_IN_SECONDS + dayPadding, endUnix + 5 * DAY_IN_SECONDS - 1.5 * dayPadding);

  return { weekGridLines, weekNumberLabels };
}

function getWeekGridLines(firstWeekUnix: number, weekCount: number) {
  return new Array(weekCount + 2).fill(0).map((_, i) => firstWeekUnix + i * WEEK_IN_SECONDS);
}

function filterWeeks(weekGridLines: number[], alternateBy: number, startThresholdUnix: number, endThresholdUnix: number) {
  return weekGridLines.filter((weekStartUnix, index) => {
    return index % alternateBy === 0 && weekStartUnix >= startThresholdUnix && weekStartUnix < endThresholdUnix;
  });
}

function getWeekRenderWidth(coordinateA?: number, coordinateB?: number): number {
