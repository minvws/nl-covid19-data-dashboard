import { endOfDayInSeconds, middleOfDayInSeconds, startOfDayInSeconds } from '@corona-dashboard/common';
import { ScaleLinear } from 'd3-scale';
import { first, last } from 'lodash';
import { TimelineEventConfig, TimelineEventXOffset } from './common';

export function getTimelineEventXOffset(config: TimelineEventConfig, xScale: ScaleLinear<number, number>): TimelineEventXOffset {
  const domain = xScale.domain();
  const min = first(domain) as number;
  const max = last(domain) as number;

  const [start, end] = [middleOfDayInSeconds(config.start), middleOfDayInSeconds(config.end || config.start)];
   ...
   * area.
   */
  const [highlightStart, highlightEnd] = [startOfDayInSeconds(config.start), endOfDayInSeconds(config.end || config.start)];
