...
>(series: T[], isMissing?: boolean) {
  return useMemo(() => {
    ...
    const getMissingDaysList = (
      startItem: SeriesSingleValue,
      endItem: SeriesSingleValue
    ) => {
      const missingDaysList: T[] = [];
      const inBetweenSeconds =
        endItem.__date_unix - startItem.__date_unix - oneDay;
      const missingDaysCount = Math.round(inBetweenSeconds / oneDay);
      const firstDay = {
        __value: startItem.__value,
        __date_unix: startItem.__date_unix,
        __hasMissing: true,
      } as SeriesMissingValue as T;
      missingDaysList.push(firstDay);
      for (let i = 1; i <= missingDaysCount; i++) {
        const nextDate = startItem.__date_unix + oneDay * i;
        const newList = {
          __value: startItem.__value,
          __date_unix: nextDate,
          __hasMissing: true,
        } as SeriesMissingValue as T;
        missingDaysList.push(newList);
      }
      const lastDay = {
        __value: endItem.__value,
        __date_unix: endItem.__date_unix,
        __hasMissing: true,
      } as SeriesMissingValue as T;
    ...
    return series.reduce<T[][]>(
      (lists, item) => {
        ...
        if (
          item.__date_unix - isLongerThanADay.__date_unix >
            DayTreshholdInSeconds &&
          (isMissing ?? false)
        ) {
