...
import { isSeriesSingleValue, SeriesDoubleValue, SeriesSingleValue, SeriesMissingValue } from './series';
 ...
 */
export function useGappedSeries<T extends SeriesSingleValue | SeriesDoubleValue | SeriesMissingValue>(series: T[], isMissing?: boolean) {
  return useMemo(() => {
    ...
    return series.reduce<T[][]>(
      (lists, item) => {
        const hasItemValue = isPresent(isSeriesSingleValue(item) ? item.__value : item.__value_a);
