 ...
 *
 */
export function useGappedLineAnnotations<T extends TimestampedValue>(values: T[], property: keyof T, label: string) {
  return useMemo(() => {
    return values.reduce<TimespanAnnotationConfig[]>((newItems, item, index, array) => {
      const value = item[property] as unknown;
      if (!hasValue(value)) {
        const startDate = (isDateValue(item) ? item.date_unix : isDateSpanValue(item) ? item.date_start_unix : NaN) - HALF_DAY_IN_SECONDS;

        const endDate = (isDateValue(item) ? item.date_unix : isDateSpanValue(item) ? item.date_end_unix : NaN) + HALF_DAY_IN_SECONDS;
         ...
         */
        if (index === array.length - 1 || hasValue(array[index + 1]?.[property])) {
