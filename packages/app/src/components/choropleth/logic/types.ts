...
export type InferedMapType<T extends ChoroplethDataItem> = T extends GmDataItem | GmEmptyItem ? 'gm' : T extends VrDataItem ? 'vr' : never;

export type InferedDataCollection<T extends ChoroplethDataItem> = T extends GmDataItem
  ? GmCollection
  : T extends GmEmptyItem
  ? GmEmptyCollection
...
export type GmEmptyCollection = { gmcode: string; empty_value: null }[];
export type GmEmptyItem = GmEmptyCollection[number];

export type ChoroplethDataItem = GmEmptyItem | GmDataItem | VrDataItem;
