...
type AccessibilityFeature = 'keyboard_time_series_chart' | 'keyboard_bar_chart' | 'keyboard_choropleth';

export type AccessibilityAnnotations = ReturnType<typeof useAccessibilityAnnotations>;
 ...
 */
export function useAccessibilityAnnotations(definition: AccessibilityDefinition) {
  const { commonTexts } = useIntl();
  const uniqueId = useUniqueId();

  const { label, description: chartDescription } = commonTexts.accessibility.charts[definition.key as keyof typeof commonTexts.accessibility.charts];

  /**
   * There needs to be a label
   */
  assert(label, `[${useAccessibilityAnnotations.name}] An accessibility label needs to be provided for ${definition.key}`);
  ...
  if (isDefined(definition.features)) {
    description.push(...definition.features.map((feature) => commonTexts.accessibility.features[feature as keyof typeof commonTexts.accessibility.features]));
  }

  const describedById = `${definition.key}_${uniqueId}`;

  return {
    descriptionElement: <VisuallyHidden id={describedById}>{description.filter(isDefined).join(' ')}</VisuallyHidden>,
    props: {
      'aria-label': label,
...
export function addAccessibilityFeatures(options: AccessibilityDefinition, additionalFeatures: AccessibilityFeature[]): AccessibilityDefinition {
  return {
    ...options,
