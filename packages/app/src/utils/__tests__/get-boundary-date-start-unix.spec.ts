...
GetBoundaryDateStartUnix('should return the start date for a timespan numberOfItems from the end of a range of date values', () => {
  ...
  assert.is(result, values[values.length - numberOfItems].date_unix - DAY_IN_SECONDS / 2);
});

GetBoundaryDateStartUnix('should return the start date for a timespan numberOfItems from the end of a range of date span values', () => {
  const values = [
  ...
  assert.is(result, values[values.length - numberOfItems].date_start_unix - DAY_IN_SECONDS / 2);
});
