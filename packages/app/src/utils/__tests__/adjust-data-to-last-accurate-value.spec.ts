...
import { adjustDataToLastAccurateValue, isValuesWithLastValue } from '../adjust-data-to-last-accurate-value';
import * as assert from 'uvu/assert';

const AdjustDataToLastAccurateValue = suite('adjustDataToLastAccurateValue');

AdjustDataToLastAccurateValue('should return the data unaltered if there are no null values', () => {
  const data = {
...
AdjustDataToLastAccurateValue('should return the data unaltered if there only null values', () => {
  const data = {
...
AdjustDataToLastAccurateValue('should adjust the last_value if there are some null values for objects', () => {
  const data = {
...
AdjustDataToLastAccurateValue('should adjust the last_value if there are some null values for values', () => {
  const data = {
    values: [1, 2, null],
    last_value: null,
  };
...
IsValuesWithLastValue('checks if the parameter is an object with values and last_value keys', () => {
