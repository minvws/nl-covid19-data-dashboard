 ...
 * TimeSeriesChart
 */
export function useSewerStationSelectPropsSimplified(data: SewerPerInstallationData) {
  const [value, setValue] = useState<string>();
  const options = useMemo(() => data.values.map((x) => ({ label: x.rwzi_awzi_name, value: x.rwzi_awzi_name })).sort((a, b) => a.label.localeCompare(b.label)), [data.values]);
