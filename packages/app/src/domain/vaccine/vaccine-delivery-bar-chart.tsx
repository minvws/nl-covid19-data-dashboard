import { colors, NlVaccineDeliveryPerSupplierArchived_20211027, NlVaccineDeliveryPerSupplierArchived_20211027Value } from '@corona-dashboard/common';
...
export function VaccineDeliveryBarChart({ data, text }: { data: NlVaccineDeliveryPerSupplierArchived_20211027; text: SiteText['pages']['vaccinations_page']['nl'] }) {
  const { commonTexts } = useIntl();
  data.values = data.values.filter((x) => !x.is_estimate);

  const productNames = text.data.vaccination_chart.product_names;

  const formatTooltip: TooltipFormatter<NlVaccineDeliveryPerSupplierArchived_20211027Value & StackedBarTooltipData> = (
    context: TooltipData<NlVaccineDeliveryPerSupplierArchived_20211027Value & StackedBarTooltipData>
  ) => {
