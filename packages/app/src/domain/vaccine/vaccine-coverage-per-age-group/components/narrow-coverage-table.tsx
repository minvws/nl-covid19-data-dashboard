...
export function NarrowCoverageTable({ values, text }: { text: SiteText['pages']['vaccinations_page']['nl']; values: NlVaccineCoveragePerAgeGroupArchivedValue[] | VrVaccineCoveragePerAgeGroupArchivedValue[] | GmVaccineCoveragePerAgeGroupArchivedValue[] }) {
    ...
    <Box>
      ...
      {values.map((item, index) => (
        <Box key={index} pt={2} pb={3} spacing={3} borderBottom="1px solid" borderColor="gray3">
          ...
          <Box spacing={1}>
            <NarrowPercentage value={'has_one_shot_percentage_label' in item ? formatCoveragePercentage(item, 'has_one_shot_percentage') : `${formatPercentage(item.has_one_shot_percentage)}%`} color={ARCHIVED_COLORS.COLOR_HAS_ONE_SHOT} textLabel={text.archived.vaccination_coverage.headers.first_shot} />
          ...
          <Box spacing={1}>
            <NarrowPercentage value={'fully_vaccinated_percentage_label' in item ? formatCoveragePercentage(item, 'fully_vaccinated_percentage') : `${formatPercentage(item.fully_vaccinated_percentage)}%`} color={ARCHIVED_COLORS.COLOR_FULLY_VACCINATED} textLabel={text.archived.vaccination_coverage.headers.coverage} />
