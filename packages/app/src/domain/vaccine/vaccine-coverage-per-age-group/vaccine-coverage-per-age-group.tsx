import { assert, GmVaccineCoveragePerAgeGroupArchivedValue, NlVaccineCoveragePerAgeGroupArchivedValue, VrVaccineCoveragePerAgeGroupArchivedValue } from '@corona-dashboard/common';
...
export function VaccineCoveragePerAgeGroup({ title, description, metadata, values, sortingOrder, text }: VaccineCoveragePerAgeGroupProps) {
  ...
  const getSortingOrder = (ageGroup: string) => {
    ...
    assert(index >= 0, `[${VaccineCoveragePerAgeGroup.name}] No sorting order defined for age group ${ageGroup}`);
  ...
  return (
    <ChartTile title={title} description={description} metadata={metadata}>
      {breakpoints.md ? <WideCoverageTable values={sortedValues} text={text} /> : <NarrowCoverageTable values={sortedValues} text={text} />}
    </ChartTile>
  );
}
