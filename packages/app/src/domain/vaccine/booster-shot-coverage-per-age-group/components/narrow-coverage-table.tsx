...
export function NarrowCoverageTable({ values, text }: { text: SiteText['pages']['vaccinations_page']['nl']; values: NlVaccineCoveragePerAgeGroupArchived_20220908Value[] | VrVaccineCoveragePerAgeGroupArchived_20220908Value[] | GmVaccineCoveragePerAgeGroupArchived_20220908Value[] }) {
    ...
    <Box>
      ...
      {values.map((item, index) => (
        <Box key={index} pt={2} pb={3} spacing={3} borderBottom="1px solid" borderColor="gray3">
          ...
          <Box spacing={1}>
            <NarrowPercentage value={'fully_vaccinated_percentage_label' in item ? formatCoveragePercentage(item, 'fully_vaccinated_percentage') : `${formatPercentage(item.fully_vaccinated_percentage)}%`} color={COLOR_FULLY_VACCINATED} textLabel={text.vaccination_coverage.headers.fully_vaccinated} />

            <Bar value={item.fully_vaccinated_percentage} color={COLOR_FULLY_VACCINATED} label={'fully_vaccinated_percentage_label' in item ? item.fully_vaccinated_percentage_label : undefined} />
          </Box>

          <Spacer mb={3} />

          <Box spacing={1}>
            <NarrowPercentage value={'booster_shot_percentage_label' in item ? formatCoveragePercentage(item, 'booster_shot_percentage') : item.booster_shot_percentage === null ? text.vaccination_coverage.no_data : `${formatPercentage(item.booster_shot_percentage)}%`} color={COLOR_FULLY_BOOSTERED} textLabel={text.archived.vaccination_coverage.headers.booster_shot} />

            <Bar value={item.booster_shot_percentage} color={COLOR_FULLY_BOOSTERED} label={'booster_shot_percentage_label' in item ? item.booster_shot_percentage_label : undefined} />
          </Box>
        </Box>
      ))}
    </Box>
