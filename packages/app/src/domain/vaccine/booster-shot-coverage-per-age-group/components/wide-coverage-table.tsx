      ...
      <StyledTable>
        ...
        >
          <Row>
            ...
            >
              <InlineText variant="label1">{text.archived.vaccination_coverage.headers.booster_shot}</InlineText>
            </HeaderCell>
            <HeaderCell
              css={css({
                width: asResponsiveArray({
            ...
            >
              <InlineText variant="label1">{text.archived.vaccination_coverage.headers.difference_booster_shot_and_fully_vaccinated}</InlineText>
            </HeaderCell>
          </Row>
        </thead>
        <tbody>
          {values.map((item, index) => (
            <Row key={index}>
              ...
              <Cell>
                <WidePercentage value={'fully_vaccinated_percentage_label' in item ? formatCoveragePercentage(item, 'fully_vaccinated_percentage') : `${formatPercentage(item.fully_vaccinated_percentage)}%`} color={COLOR_FULLY_VACCINATED} justifyContent="flex-end" />
              </Cell>
              <Cell>
                <WidePercentage value={'booster_shot_percentage_label' in item ? formatCoveragePercentage(item, 'booster_shot_percentage') : item.booster_shot_percentage === null ? text.vaccination_coverage.no_data : `${formatPercentage(item.booster_shot_percentage)}%`} color={COLOR_FULLY_BOOSTERED} justifyContent="flex-end" />
              </Cell>
              <Cell>
                <Box spacing={1}>
