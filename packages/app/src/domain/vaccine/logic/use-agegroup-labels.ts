import { GmVaccineCoveragePerAgeGroupValue, VrVaccineCoveragePerAgeGroupValue } from '@corona-dashboard/common';
import { useMemo } from 'react';
import { isDefined } from 'ts-is-present';
import { useIntl } from '~/intl';
import { getRenderedVaccinatedLabel } from './parse-vaccinated-percentage-label';

export function useAgegroupLabels(dataValue: VrVaccineCoveragePerAgeGroupValue | GmVaccineCoveragePerAgeGroupValue | undefined, lowerCased?: boolean) {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      autumn_2022_vaccinated_percentage: lowerCased ? oneShotLabel.toLocaleLowerCase() : oneShotLabel,
    };
  }, [dataValue, labelsText.meer_dan, labelsText.minder_dan, formatPercentage, lowerCased]);
}
